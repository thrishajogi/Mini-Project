// server.js — ZeroBank (Atlas-only) — permanent-lock + admin unlock
...
const MAX_ATTEMPTS = parseInt(process.env.MAX_ATTEMPTS || '3', 10); // number of failed tries before permanent lock
...
// LOGIN (risk engine, mfa) — modified to enforce permanent lock
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    // If permanently locked, tell user to contact backend/admin
    if (user.isLocked) {
      await logEvent('login_blocked_locked', { email, ip, ua, details: {} });
      return res.status(429).json({ success: false, error: 'Account locked. Contact backend/admin to unlock.' });
    }
    ...
    if (!match) {
      ...
      // If threshold reached -> permanent lock
      if (user.failedAttempts >= MAX_ATTEMPTS) {
        user.isLocked = true;
        // keep failedAttempts for auditing
        await user.save();

        await logEvent('account_locked', { email, ip, ua, details: { maxAttempts: MAX_ATTEMPTS } });
        await sendAlertEmail(user.email, 'ZeroBank — Account locked', `<p>Your account was locked after too many failed login attempts. Contact support to unlock your account.</p>`);

        return res.status(429).json({ success: false, error: `Too many failed attempts. Account locked. Contact backend/admin to unlock.` });
      }
    ...
    // success path: reset failedAttempts, update device info
    ...
    // decide MFA requirement (risk & setup)
    const hasSecurity = (user.totpSecret && user.totpSecret.length > 0) ||
                        (user.securityQuestion && (user.securityQuestion.answer || '').trim().length > 0);
...
// NEW: admin unlock endpoint — POST /admin/unlock { email, secret }
app.post('/admin/unlock', async (req, res) => {
  try {
    const { email, secret } = req.body || {};
    if (!ADMIN_SECRET) return res.status(500).json({ success: false, error: 'Admin unlock not configured' });
    if (!secret || secret !== ADMIN_SECRET) return res.status(403).json({ success: false, error: 'Forbidden' });
    if (!email) return res.status(400).json({ success: false, error: 'Email required' });
    ...
    await logEvent('admin_unlock', { email, ip: req.ip, ua: req.headers['user-agent'], details: { by: 'admin' } });
